...
public class YamlLoader {
    ...
    /**
     * Loads a list of object from a yaml file.
     *
     * @param clazz The class of the object that should be loaded.
     * @param <T>   The generic type that the method should return.
     * @return The list of objects with the properties loaded from the provided yaml file.
     */
    public <T> List<T> loadList(Class<T> clazz) throws IOException {
        return MAPPER.readValue(path.toFile(), MAPPER.getTypeFactory().constructCollectionType(List.class, clazz));
    }
}